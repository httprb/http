...
module HTTP
  ...
  class Connection
    ...
    attr_reader :pending_request, :pending_response
    ...
    def initialize(req, options)
      ...
      if req.uri.is_a?(URI::HTTPS) && !req.using_proxy?
        start_tls(req.uri.host, options[:ssl_socket_class], options[:ssl_context])
      end
    end
    ...
    def send_request(req)
      if pending_request
        raise StateError, "Tried to send a request while one is pending already. This cannot be called from multiple threads!"
      elsif pending_request
        raise StateError, "Tried to send a request while a response is pending. Make sure you've fully read the body from the request."
      end
    ...
    def set_keep_alive
      return @keep_alive = true unless persistent

      # HTTP/1.0 requires opt in for Keep Alive
      if parser.http_version == '1.0'
        @keep_alive = parser.headers? && parser.headers['connection'] == HTTP::Client::KEEP_ALIVE
